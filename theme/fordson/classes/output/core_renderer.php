...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function full_header() {
        ...
        // Elo: Get pagetype
        $header->elocard = $this->elo_get_pagetype();
...
' . $jitsibuttontext . ' </span></a>';
    ...
    public function headerimage() {
        ...
        if (empty($courseimage) && !isset($COURSE->image)){
            ...
            $headerbg = $courseimage;
            $headerbgimgurl = $courseimage;
        }
        $defaultimgurl = $OUTPUT->image_url('headerbg', 'theme');        
        if(!$headerbg){
            $headerbg = $CFG->wwwroot. '/theme/fordson/pix/headerbg.png';
        }
        if(!$headerbgimgurl){
        ...
        {
            $courseimageid  = $COURSE->id % 10;
            $courseimage = $CFG->wwwroot. '/theme/fordson/pix/courseimages/courseimage'. $courseimageid . '.jpg';          
        }                  
        // End Elo
    ...
    */
    ...
    public function teacherdash() {
        ...
        $progress = $this->render_from_template('theme_fordson/progress-chart', $progresschartcontext);
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        $context->elocarddefaultlogin = $this->elo_get_pagetype();
        $url = $this->get_elologo_url($context->elocarddefaultlogin);
        // Custom logins.
        // End Elo
        ...
        // Elo: elocarddefaultlogin, show/hide guidelines
        $context->displayguidelines = $PAGE->theme->settings->guidelines == 1;
        $context->elocarddefaultlogin = $this->elo_get_pagetype();

        return $this->render_from_template('core/loginform', $context);
    }
    // Elo
    public function get_elologo_url($idelopage,$maxwidth = null, $maxheight = 200) {
        global $CFG;
        $idelopage == 'site-index' ? $elo_login_logo = $CFG->wwwroot. '/theme/fordson/pix/headerlogo.png' :
        $elo_login_logo = $CFG->wwwroot. '/theme/fordson/pix/logoblue.png';//Nhien elo 
        return new moodle_url($elo_login_logo);        
    }

    public function get_compact_logo_url($maxwidth = 100, $maxheight = 100) {
        global $CFG;
        $elo_compact_logo = parent::get_compact_logo_url(null, 70);
        if(!$elo_compact_logo){
        $elo_compact_logo = $CFG->wwwroot. '/theme/fordson/pix/navlogo.png';}
        return $elo_compact_logo;
    }

    public function elo_get_pagetype() {
        $elopagetypeclass = $this->page->__get('pagetype');
        return $elopagetypeclass;
    }
    // End Elo
