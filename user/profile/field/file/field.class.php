...
class profile_field_file extends profile_field_base
{
    ...
    public function display_data()
    {
        ...
        foreach ($files as $file) {
            ...
            if ($this->field->name == 'Hình cover') { // Nhien elo hinhcover 02_10_2019
                $url = file_encode_url("$CFG->wwwroot/pluginfile.php", $path, false);
                $filename = $file->get_filename();
                $data[] = html_writer::img($url, $filename, array('width' => '50%'));
            } else {
    ...
    public function get_cover_image_path()
    {
        global $CFG;
        // Default formatting.
        $data = parent::display_data();
        ...
        foreach ($files as $file) {
            $path = '/' . $context->id . '/profilefield_file/files_' . $this->fieldid . '/' .
                ...
                $file->get_filepath() .
                $file->get_filename();
            if ($this->field->name == 'Hình cover') {
                $url = file_encode_url("$CFG->wwwroot/pluginfile.php", $path, false, false);
                return $url;
            }
            return $data;
        }
